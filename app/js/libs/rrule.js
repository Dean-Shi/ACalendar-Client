...
(function(root, factory) {
  ...
  if (typeof exports !== 'undefined') {
    // Node/CommonJS
    module.exports = {
        RRule: factory(root, require('underscore'))
    }
  } else if (typeof define === 'function' && define.amd) {
    // AMD
    define(['underscore'], function(_) {
      // Export global even in AMD case in case this script is loaded with
      // others that may still expect a global RRule.
      return root.RRule = factory(root, _);
    });
  } else {
    // Browser globals
    root.RRule = factory(root, root._);
  }
...
Weekday.prototype = {
    ...
    // __repr__
    toString: function() {
        var s = ['SU', 'MO', 'TU', 'WE', 'TH', 'FR', 'SA'][this.weekday];
        if (this.n) {
...
var RRule = function(options, noCache) {
    ...
    if (!(plb(opts.byweekno) || plb(opts.byyearday)
        || plb(opts.bymonthday) || plb(opts.byweekday)
        || opts.byeaster !== null))
    {
        switch (opts.freq) {
            case RRule.YEARLY:
...
RRule.SU = new Weekday(0);
RRule.MO = new Weekday(1);
RRule.TU = new Weekday(2);
RRule.WE = new Weekday(3);
RRule.TH = new Weekday(4);
RRule.FR = new Weekday(5);
RRule.SA = new Weekday(6);
